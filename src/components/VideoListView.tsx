...
export function VideoListView(props: IVideoListViewProps) {
    ...
    return (<div className={'space-y-4'}>
            <input className={'p-4 w-full rounded-lg cursor-pointer text-gray-800 bg-white hover:bg-gray-50'}
                   placeholder={"Search videos"} onChange={handleSearch}/>
            {currentResults.map((item) => (<div
                    key={item.id.videoId ? item.etag + item.id.videoId : item.etag + item.id.channelId}
                    onClick={() => handleItemClicked(item.id.videoId)}
                    className={`p-4 rounded-lg cursor-pointer transition-colors ${activeVideo === item.id.videoId ? 'bg-blue-100 border-2 border-blue-500' : 'bg-white hover:bg-gray-50'}`}
                >
            ...
            {searchResults.length ? <Pagination itemsPerPage={itemsPerPage} totalItems={searchResults.length}
                                                onPageChange={handlePageSelection} currentPage={currentPage}/> : null}
        </div>);
}
